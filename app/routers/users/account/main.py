...
def read_users(db: Session = Depends(get_db), is_validated=Depends(validate_bearer)):
...
async def get_user_by_email(email: str, db:Session = Depends(get_db), is_validated=Depends(validate_bearer)):
...
async def get_user_by_reset_password_token(token: str, db:Session = Depends(get_db), is_validate=Depends(validate_bearer)):
...
async def update_user_After_Reset_Password(updateStaff: UpdateStaff, db:Session = Depends(get_db), is_validated=Depends(validate_bearer)):
